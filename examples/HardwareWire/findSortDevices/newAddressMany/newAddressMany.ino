...
//   Начинаем работу с модулями:                             //   Указанные действия можно выполнить в цикле.
     j[0].begin(&Wire); // &Wire1, &Wire2 ...                //   Функция begin() принимает ссылку на объект для работы с шиной I2C и проверяет существование модуля на этой шине.
     j[1].begin(&Wire); // &Wire1, &Wire2 ...                //   К функции begin() можно обратиться с проверкой: if( j[1].begin(&Wire) ){/*модуль обнаружен*/;}else{/*нет*/;}
     j[2].begin(&Wire); // &Wire1, &Wire2 ...                //   После begin(), результат обнаружения модуля остаётся доступным для чтения: if( j[2] ){/*модуль был обнаружен*/;}else{/*нет*/;}
     j[3].begin(&Wire); // &Wire1, &Wire2 ...                //   Если модули находятся на основной аппаратной шине I2C (под управлением объекта Wire), то ссылку на объект можно не указывать: j[3].begin();
