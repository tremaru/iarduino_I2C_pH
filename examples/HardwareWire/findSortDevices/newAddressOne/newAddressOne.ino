...
iarduino_I2C_Address j;                                      //   Создаём объект j для работы с модулем I2C-flash. Адрес модуля будет определен автоматически.
...
//   Начинаем работу с модулем:                              //
     j.begin(&Wire); // &Wire1, &Wire2 ...                   //   Функция begin() принимает ссылку на объект для работы с шиной I2C и проверяет существование модуля на этой шине.
//   Выводим сообщение о отсутствии модуля:                  //   Получить результат проверки можно сразу: if( j.begin(&Wire) ){...}, а можно так, как показано в этом примере.
